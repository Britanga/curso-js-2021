...
if (indexedDB && form) { // Si indexedDB y form existe
    ...
    const deleteData = (key) => {
        const transaction = db.transaction(['tasks'], 'readwrite')
        const objectStore = transaction.objectStore('tasks')
        const request = objectStore.delete(key) // Borra titulo y prioridad de la task
        request.onsuccess = () => {
            readData()
        }
    }
    ...
    const readData = () => {
        ...
        request.onsuccess = (e) => {
            ...
            if (cursor) { // Si en el cursor hay datos q leer
                ...
                taskDelete.dataset.type = 'delete' // Añadiendo data-type="delete"
                taskDelete.dataset.key = cursor.key // Añadiendo data-key="titulodeltask"
    ...
    tasks.addEventListener('click', (e) => {
        ...
        } else if (e.target.dataset.type == 'delete') { // Al clickear en el boton de delete
            deleteData(e.target.dataset.key) // Invoca deleteData y pasa como parametro la key
        }
    })
}