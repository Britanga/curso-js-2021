...
form.addEventListener('submit', (e) => {
    ...
    // sessionStorage.setItem('person', JSON.stringify(person)) // Guarda el objeto en string
    // sessionStorage.setItem('name', 'Dorian') // Guarda nombre

    localStorage.setItem(form.key.value, form.value.value) // Guarda Key y Value

    keys.innerHTML += `<option>${ form.key.value }</option>` // Agregando option en el select

    form.reset() // Vacia los campos del form cuando guardamos con el boton save
})

keys.addEventListener('change', () => {
    // Agrega el contenido de Value en el elemento p q tiene id infoValue del html
    document.getElementById('infoValue').textContent =
    // Devuelve el contenido de Value con respecto al Key escogido
        localStorage.getItem(keys[ keys.selectedIndex ].textContent)
})

// localStorage.clear() // borra todos los Keys y Value
